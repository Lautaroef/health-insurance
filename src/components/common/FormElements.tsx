...
const TextInput = ({ name, label, placeholder, showError, errorMessage }: TextInputValues) => {
  return (
    <>
      ...
      {showError && errorMessage ? <span className="form-error">{errorMessage}</span> : null}
    </>
  );
};

const DateInput = ({ name, label, placeholder, showError, errorMessage }: DateInputValues) => {
  return (
    <>
      <div className="form-container">
        <label className="label" htmlFor={name}>
  ...
  return (
    <>
      <div className="form-container-group">
        <Field className="form-container-group__select" name={selectName} component="select">
...
const CheckboxInput = ({ children, name, showError, errorMessage }: CheckboxInputValues) => {
  return (
    <>
      <div className="mt-20">
    ...
    <>
      <div className={`form-container-group ${className}`}>
        ...
        <div className="form-container-group__input--right">
          ...
          <Field id={dateName} name={dateName} type="date" placeholder={datePlaceholder} />
        </div>
      </div>
      {showError && errorMessage ? <span className="form-error">{errorMessage}</span> : null}
    </>
